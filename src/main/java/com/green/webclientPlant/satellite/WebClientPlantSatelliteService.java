...
@Service
@Slf4j
public class WebClientPlantSatelliteService {
    private final WebClient webClient;
    private final String key;
    private final WebClientPlantSatelliteMapper mapper;

    @Autowired
    public WebClientPlantSatelliteService(@Value("${properties.key}") String key, WebClientPlantSatelliteMapper mapper) {
        this.mapper = mapper;
    ...
    public List<PlantSatelliteEntity> getPlant(PlantSatelliteGetReq p) {
        System.out.println(p);
        String json = null;
        String uriString = String.format("http://openapi.nature.go.kr/openapi/service/rest/PlantService/seedSearch?serviceKey=%s&_type=%s&numOfRows=%s&pageNo=%s",
        ...
        List<PlantSatelliteEntity> testList = null;

        try{
            JsonNode node = om.readTree(json);
            testList = om.convertValue(node.at("/response/body/items/item"),
                    new TypeReference<List<PlantSatelliteEntity>>(){});
        } catch (Exception e){
